...
// 1. Create send-staff-welcome API
...
fs.mkdirSync('src/app/api/send-staff-welcome', { recursive: true });
fs.writeFileSync('src/app/api/send-staff-welcome/route.ts', apiContent);
console.log('✓ Welcome email API created');

// 2. Trigger welcome email from staff-management.tsx on account creation
let mgmt = fs.readFileSync('src/components/staff-management.tsx', 'utf8');
mgmt = mgmt.replace(
...
fs.writeFileSync('src/components/staff-management.tsx', mgmt);
console.log('✓ Staff management updated to send welcome email');
