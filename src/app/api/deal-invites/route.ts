...
async function getInvites() {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'https://lease.orsone.app'}/api/registered-leads`);
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'https://lease.orsone.app'}/api/registered-leads`, {
      method: 'POST',
