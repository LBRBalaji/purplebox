...
export function OrsTransactManager() {
  ...
  // ── All state declarations at the top — never reordered, never conditional ──
  ...
  // ── Single return — use conditional rendering, never conditional returns ──
  return (
    <div className="mt-4">
      {editing ? (
        /* Edit view */
            ...
            <div style={{ flex: '1 1 220px', display: 'flex', alignItems: 'center', gap: 8, background: 'var(--color-background-primary)', border: '0.5px solid hsl(259 30% 85%)', padding: '6px 10px' }}>
              ...
              {searchInput && (
                <button onClick={() => { setSearchInput(''); setSearch(''); }}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'hsl(259 15% 55%)', padding: 0 }}>
            ...
            <>
                ...
                {listings.map((l, i) => (
                  <div key={l.id || l.ors_property_id}
                    ...
                    <p style={{ fontSize: 11, color: 'var(--color-text-primary)', margin: 0 }}>
                      {l.lease_area_as_advertised_in_sq_ft
                        ? Number(l.lease_area_as_advertised_in_sq_ft).toLocaleString()
              ...
              {totalPages > 1 && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, paddingTop: 12, flexWrap: 'wrap' }}>
                  ...
                  {pageNumbers.map((p, i) =>
                    p === '...'
                      ? <span key={`e${i}`} style={{ padding: '5px 4px', fontSize: 12, color: 'hsl(259 15% 55%)' }}>…</span>
