...
function DownloadBar() {
    ...
    const handleDownload = () => {
        ...
        let failedLocations = new Set<string>();
        ...
        if (failedLocations.size > 0) {
            const locationNames = Array.from(failedLocations).join(', ');
            toast({
                variant: 'destructive',
                title: 'Download Limit Exceeded',
                description: `You have reached your daily download limit for the following location(s): ${locationNames}. You can download more after 24 hours.`
            });
        }
    ...
    const handleLoginSuccess = () => {
        ...
        // After login, re-trigger the download attempt
        // A small delay might be needed if user data isn't immediately available
                    ...
                    <Button onClick={handleDownload}>
                         {user ? (
                            <>
                             ...
                             <>
                                <LogIn className="mr-2 h-4 w-4" /> Login to Download
                            </>
                        )}
                    </Button>
                </div>
...
export default function ListingsPage() {
  ...
  const { selectedForDownload, toggleSelectedForDownload, getTodaysDownloadsForLocation } = useData();
  ...
  const handleSelectionChange = useCallback((listing: WarehouseSchema) => {
    if (!user) {
        // If user is not logged in, allow selection but the download bar will prompt login
        toggleSelectedForDownload(listing);
        return;
    }
    
    // Check if the user is selecting or de-selecting
    const isCurrentlySelected = selectedIds.has(listing.id);
    
    if (!isCurrentlySelected) {
        // This is a new selection, check the limit
        const downloadsToday = getTodaysDownloadsForLocation(user.email, listing.locationName);
        const selectedFromLocation = selectedForDownload.filter(l => l.locationName === listing.locationName).length;

        if (downloadsToday + selectedFromLocation >= 3) {
            toast({
                variant: 'destructive',
                title: 'Daily Download Limit Warning',
                description: `You can only select up to 3 properties per location per day. You have already reached this limit for "${listing.locationName}".`,
            });
            return; // Prevent selection
        }
    }
    
    // If checks pass, toggle the selection
    toggleSelectedForDownload(listing);

  }, [user, toast, selectedIds, selectedForDownload, getTodaysDownloadsForLocation, toggleSelectedForDownload]);
        ...
        <div className="max-w-7xl mx-auto">
            <div className="mb-8 rounded-lg border bg-card text-card-foreground shadow-sm p-6">
                ...
                <div className="mt-6 flex flex-col md:flex-row gap-4">
                 ...
                 {user?.role === 'User' && (
                    <div className="mt-4 p-3 rounded-md bg-blue-50 text-blue-800 border border-blue-200 text-sm flex items-start gap-3">
                        <AlertTriangle className="h-4 w-4 mt-0.5 shrink-0" />
                        <div>
                            You can select and download details for up to <strong>3 properties per location</strong> in a 24-hour period.
                        </div>
                    </div>
                )}
            </div>
